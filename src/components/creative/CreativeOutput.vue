...
import { Loading, Document, Magnet } from '@element-plus/icons-vue'
...
<template>
  <div class="creative-output h-full">
    ...
    <div v-if="loading" class="output-loading flex flex-col gap-4">
      <div class="loading-header flex items-center gap-2 text-purple-300 font-semibold">
      ...
      <div class="grid grid-cols-2 gap-4 mt-2">
        <div v-for="i in 4" :key="i" class="skeleton-card space-y-3 p-4 rounded-lg bg-white/5">
    ...
    <div v-else-if="scripts.length === 0" class="output-empty flex flex-col items-center justify-center h-full text-center">
      <div class="empty-icon text-slate-600">
        <el-icon :size="64"><Document /></el-icon>
      </div>
      <p class="empty-title text-lg font-semibold text-slate-200 mt-4">等待生成创意文案</p>
      <p class="empty-desc text-sm text-slate-400 mt-1 max-w-xs">
    ...
    <div v-else class="output-grid flex flex-col gap-4 h-full">
      <div class="output-header flex items-center justify-between">
        <h3 class="output-title flex items-center gap-2 text-base font-semibold text-slate-200">
          <el-icon :size="16"><Magnet /></el-icon>
          <span>生成结果</span>
        </h3>
        <span class="output-count text-xs font-medium text-slate-400 bg-white/5 px-2 py-1 rounded-full">
      ...
      <div v-if="selectedId" class="output-action text-center p-2">
        <p class="action-hint text-sm font-medium text-green-400">
          已选择方案，点击右下角按钮进入音画配置
        </p>
      </div>
    </div>
